    ...
    switch (event) {
        ...
        case 'deleted':
            if (!dup_event) {
                ...
                if (empty) {
                    myPlaylist.restore_poster_list(); // re-enable poster-list display (if defined)
                    if (removedItem !== null) {
...
function initCdPosters2(cd_parms) {
    ...
    if (cd_parms.poster_list) {
        if (cd_parms.poster_list.length > 0) {
            ...
            wants_who = cd_parms.wants_who;

            have_posterlist = true;
        }
    }
    ...
    setTimeout(function () {
        ...
        if (have_posterlist) {
            ...
            if (wants_who) {
                myPlaylist.set_post_list_auto_override();
            }
        }
        ...
        if (have_posterlist) {
            for (var j = 0; j < cd_parms.poster_list.length; j++) {
                var art_work = cd_parms.poster_list[j];
                fetchImage(art_work);
            }
        }
...
function initPlaylist(art_work, start_with_video) {
    ...
    //var video_play = $('.jp-video-play-icon');

    //if (video_play.length > 0 && added_photo) {
    //    video_play.css('background', added_photo);
    //}
...
function songIsShown(n) {
    ...
    try {
        ...
        var errmsg = "theFsongs[" + n + "].divobj.is(':visible') threw " + err.message;
        console.log("filter_mobile::songIsShown .. " + errmsg);
    }
    return false;
}
...
function playRandomSong() {
    ...
    if (notPlayed.length <= 0) {
        ...
        //  It's too dangerous to do this, because it's impossible to know what has been played
        //  on the playlist and what has not been played.   Playlist could contain only Intro
        //  which user does not want to hear.  If nothing has been played, this routine will
        //  keep selecting the first number on the playlist and nothing will seem to happen.
